...
screen file_manager_ui():
    ...
    frame:
        ...
        if scenario_picker2 or scenario_picker1:
            # --- TAPE 1 (LEFT) ---
            vbox:
                anchor (0.5, 0.5)
                pos (300, 400) 
                spacing 10
                imagebutton:
                    idle Transform("images/ui/tape_icon.png", zoom=0.30)
                    action [Hide("file_manager_ui"), SetVariable("tape_num", 1), Jump("cctv_tape_view")]
                    xalign 0.5
            ...
            vbox:
                ...
                imagebutton:
                    idle Transform("images/ui/tape_icon.png", zoom=0.30)
                    action [Hide("file_manager_ui"), SetVariable("tape_num", 2), Jump("cctv_tape_view")]
                    xalign 0.5
            ...
            vbox:
                ...
                imagebutton:
                    idle Transform("images/ui/tape_icon.png", zoom=0.30)
                    action [Hide("file_manager_ui"), SetVariable("tape_num", 3), Jump("cctv_tape_view")]
                    xalign 0.5
...
label setup_hack_level:
    ...
    python:
        hack_snippets = []
        for i in range(4):
            hack_snippets.append({"id": i, "img": f"images/puzzle/fingerprint/Tile/correct_{hack_level}_{i}.png", "correct": True, "clicked": False})
        for i in range(4):
            hack_snippets.append({"id": i+4, "img": f"images/puzzle/fingerprint/Tile/wrong_{hack_level}_{i}.png", "correct": False, "clicked": False})
        
        import random
        random.shuffle(hack_snippets)
        target_fp = f"images/puzzle/fingerprint/Correct/target_fp_{hack_level}.png"
...
screen hacking_minigame():
    ...
    hbox:
        align (0.5, 0.5)
        spacing 150 # Adjust this number to change the space between the grid and the target image
        
        # --- LEFT SIDE: Timer & Grid ---
        vbox:
            spacing 30
            
            vbox:
                spacing 5
                text "CONNECTION TIMEOUT" size 20 color "#FFF"
                text "[hacking_timer] SEC" size 50 color "#F00"
                text "COMPONENTS DETECTED: [hack_found] / 4" size 20 color "#FFF"
            ...
            grid 2 4:
                ...
                for snippet in hack_snippets:
                    if not snippet["clicked"]:
                        imagebutton:
                            idle Transform(snippet["img"], zoom=0.5)
                            hover Transform(snippet["img"], zoom=0.5, matrixcolor=BrightnessMatrix(0.2))
                            action [
                        ...
                        if snippet["correct"]:
                            add Transform(snippet["img"], zoom=0.5, matrixcolor=TintMatrix("#0F0"))
                        else:
                            add Transform(snippet["img"], zoom=0.5, matrixcolor=TintMatrix("#F00"))
                            
        # --- RIGHT SIDE: Target Fingerprint ---
        vbox:
            spacing 20
            yalign 0.5
            text "CLONE TARGET" size 20 color "#FFF" xalign 0.5
            
            # If the target fingerprint is also too big, you can add zoom here too!
            add Transform(target_fp, zoom=0.8) xalign 0.5 
...
screen interrogation_room():
    modal True
    add "images/interrogation_bg.png" 
    text "INTERROGATION ROOM" size 40 color "#FFF" align (0.5, 0.05)

    # DAN
    imagebutton:
        focus_mask True
        idle "images/suspects/dan_interro.png"
        hover Transform("images/suspects/dan_interro.png", matrixcolor=BrightnessMatrix(0.2))
        insensitive Transform("images/suspects/dan_interro.png", matrixcolor=SaturationMatrix(0.0))
        action If(not interrogated_dan, Jump("interrogate_dan"), NullAction())

    # TOPH
    imagebutton:
        focus_mask True
        idle "images/suspects/toph.png"
        hover Transform("images/suspects/toph.png", matrixcolor=BrightnessMatrix(0.2))
        insensitive Transform("images/suspects/toph.png", matrixcolor=SaturationMatrix(0.0))
...
screen final_accusation():
    ...
    # 1. Stack the full-size transparent images first
    add "images/suspects/dan_interro.png"
    add "images/suspects/toph.png"
    add "images/suspects/austin.png"
    add "images/suspects/chandler.png"
