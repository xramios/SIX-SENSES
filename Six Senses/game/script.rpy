...
label tutorial:
    ...
    s "To proceed to the next day, click on the elevator."
...
label day3intro:
    ...
    if scenario_picker1:
        ...
        mc "Good. I'll review them immediately."
    $ seen_evidence_room_intro = False
    show text "{size=70}Sense Activated — TOUCH{/size}" at truecenter with dissolve
    pause 2.0
    hide text with dissolve
...
label evidence_room_hub:
    $ current_location = "evidence_room_hub"
    ...
    if not seen_evidence_room_intro:
        ...
        "You can choose to click the bag, to look over your items, or click the corkboard to check your Journal."
        if current_day >= 3:
...
label phone_app_messages:
    ...
    show image "images/phone/msg_1.png" at Transform(zoom=0.5, align=(0.5, 0.5)) with dissolve
    ...
    show image "images/phone/msg_2.png" at Transform(zoom=0.5, align=(0.5, 0.5)) with dissolve
    mc "Demanding money, claiming a pregnancy scare..." 
    
    show image "images/phone/msg_3.png" at Transform(zoom=0.5, align=(0.5, 0.5)) with dissolve
    mc "...threatening to expose his drug use to his coach and even his father."
...
label cctv_tape_view:
    ...
    # ==========================================
    # PATH: CHECKED CCTV ROOM FIRST (3 Clear Tapes)
    # ==========================================
    if scenario_picker1 and not scenario_picker2:
        if tape_num == 1:
            show text "{size=50}TAPE: DAN LEADING PAT TO THE STORAGE ROOM{/size}" at truecenter with dissolve
            pause 2.0
            scene cctv_1_reveal with fade 
            "The footage shows Dan gripping Pat firmly by the hand, pulling him down the hallway with urgency. Pat resists slightly, stumbling to keep up as he tries to pull back."
            "Dan doesn’t let go, tightening his grip and dragging Pat toward the storage room. Without hesitation, Dan yanks the door open and pulls Pat inside. The door shuts quickly behind them."
            
            $ add_suspect("Dan (Janitor)", "Janitor seen leading victim to storage room on CCTV.", "images/characters/danicon.png")
            $ record_clue("Dan (Janitor)", "CCTV Tape 1|Footage shows Dan forcing Pat into the storage room.")
            s "Clue Recorded."

        elif tape_num == 2:
            show text "{size=50}TAPE: TOPH AT THE ELEVATOR{/size}" at truecenter with dissolve
            pause 2.0
            scene cctv_4_reveal with fade 
            "The tape shows Toph suspiciously waiting inside the elevator while holding his phone."
            
            $ add_suspect("Toph", "Seen on CCTV near the crime scene on the night of the incident.", "images/characters/toph.png")
            $ record_clue("Toph", "CCTV Tape 2|Seen nervously waiting in the elevator.")
            s "Clue Recorded."

        elif tape_num == 3:
            show text "{size=50}TAPE: CHANDLER ON CALL — AUSTIN PEEKING{/size}" at truecenter with dissolve
            pause 2.0
            scene cctv_chandler with fade 
            "The tape shows Chandler walking through the hallway, phone pressed to his ear, heading toward the storage room. As he approaches, Austin slowly leans out from behind the corner, cautiously peeking in Chandler's direction."
            "Chandler enters the storage room and disappears from view. Austin lingers for a second, scanning the hallway as if making sure it's clear, before stepping out slightly further — still keeping part of his body hidden."
            mc "Ahhh, so this was the one the officers took."
            
            $ add_suspect("Chandler", "Seen on CCTV near the crime scene.", "images/characters/chandler.png")
            $ add_suspect("Austin", "Seen tailing Chandler on CCTV.", "images/characters/austin.png")
            $ record_clue("Chandler", "CCTV Tape 3|Seen on a call heading into the storage room.")
            $ record_clue("Austin", "CCTV Tape 3|Seen peeking around the corner and tracking Chandler's movements.")
            s "Clues Recorded."

    # ==========================================
    # PATH: CHECKED THE BODY FIRST (Corrupted Tapes)
    # ==========================================
    elif scenario_picker2:
        if tape_num == 1:
            show text "{size=50}CORRUPTED TAPE 1{/size}" at truecenter with dissolve
            pause 2.0
            scene corr_cctv_dan with fade
            "Security footage recovered from the camera shows partial visual data of the corridor leading to the storage room. The recording is corrupted and intermittently fragmented, with missing frames and visual distortion."
            "Within the salvageable segments, a figure is seen moving toward the storage room."
            
            $ record_clue("Pat (Victim)", "CCTV Tape 1|Corrupted footage shows an unknown figure moving toward the storage room.")
            s "Clue Recorded."

        elif tape_num == 2:
            show text "{size=50}CORRUPTED TAPE 2{/size}" at truecenter with dissolve
            pause 2.0
            scene cctv_toph with fade 
            "The tape was recovered, but the footage is heavily corrupted. Static and skipping frames obscure most of the image — though a figure is still visible inside the elevator, holding a phone."
            
            $ record_clue("Pat (Victim)", "CCTV Tape 2|Corrupted footage shows an unknown figure waiting in the elevator.")
            s "Clue Recorded."
            ...
            # Identities are clear here, so suspects are added normally
            $ add_suspect("Chandler", "Seen on CCTV near the crime scene.", "images/characters/chandler.png")
            $ add_suspect("Austin", "Seen tailing Chandler on CCTV.", "images/characters/austin.png")
            $ record_clue("Chandler", "CCTV Tape 3|Seen on a call heading into the storage room.")
            $ record_clue("Austin", "CCTV Tape 3|Seen peeking around the corner and tracking Chandler's movements.")
...
label day5_call_logs:
    ...
    $ listened_pat_chandler = False
    $ listened_dan_toph = False
    
    "You put your headphones on."
    mc "Finally, let's hear what they said."

label call_log_menu:
    scene black
    menu:
        "CALL LOG: PAT TO CHANDLER" if not listened_pat_chandler:
            jump log_pat_chandler
            
        "CALL LOG: DAN TO TOPH" if not listened_dan_toph:
            jump log_dan_toph
            
        "Return to Precinct" if listened_pat_chandler and listened_dan_toph:
            jump finish_call_logs

label log_pat_chandler:
    scene black with dissolve
    show text "{size=50}CALL LOG: PAT TO CHANDLER{/size}" at truecenter with dissolve
    pause 2.0
    hide text with dissolve

    # Add your actual audio file path here
    play sound "audio/pat_chandler_call.mp3"

    # Adjust the numbers in the {w=...} tags to sync with your audio track
    "Pat (recording)" "Hon, where are you right now?{w=6}{nw}"
    
    "Chandler (recording)" "Huh? I'm at the airport. Why? Where are you?{w=10}{nw}"
    
    "Pat (recording)" "audible crying sounds{w=9.0}{nw}"
    
    "Chandler (recording)" "It's already past 9, huh. Haven't they let you out yet?{w=5.5}{nw}"
    "Chandler (recording)" "Wait — hold on, are you crying?{w=6.0}{nw}"
    
    "Pat (recording)" "I'm here now in… in the storage room. I need your help, hon.{w=10}{nw}"
    
    "Chandler (recording)" "Huh?{w=1.0}{nw}"
    "Chandler (recording)" "Wait — I'm coming up.{w=4.0}{nw}"
    
    stop sound
    
    $ record_clue("Chandler", "Audio Evidence|Call log confirms Pat called him from the storage room crying, and he said he was 'coming up'.")
    
    $ listened_pat_chandler = True
    jump call_log_menu

label log_dan_toph:
    scene black with dissolve
    show text "{size=50}CALL LOG: DAN TO TOPH{/size}" at truecenter with dissolve
    pause 2.0
    hide text with dissolve

    # Add your actual audio file path here
    play sound "audio/dan_toph_call.mp3"

    "Timestamp reads 9:00 PM.{w=1.0}{nw}"
    
    "Toph" "What's the problem?{w=1.0}{nw}"
    
    "Dan" "Where is it, then?{w=1.0}{nw}"
    
    "Toph" "Pat has the money.{w=1.0}{nw}"
    
    "Dan" "No, it's gone. I'm throwing you under the bus. All your bullshit, everything you've been doing…{w=7.5}{nw}"
    "Dan" "What's your father going to do to you when he finds out about all this, huh?{w=7.0}{nw}"
    
    "Toph" "What… You didn't get the money? What are you talking about? Come on, I'll add more. How much do you want?{w=9.0}{nw}"
    
    "Dan" "I don't need your money anymore.{w=5.0}{nw}"
    
    "Toph" "Oh, really? Well, if you're throwing me under the bus — who says I can't do the same to you?{w=9.0}{nw}"
    
    "Dan" "Get to the 6th floor of TYK. This is your fault, Toph. Fix it.{w=6.0}{nw}"
    
    "Toph" "What are you talking about?{w=2.0}{nw}"
    
    stop sound
    
    $ record_clue("Dan (Janitor)", "Audio Evidence|Demanded Toph come to the 6th floor after an argument over missing money.")
    $ record_clue("Toph", "Audio Evidence|Argued with Dan over missing money and was told to go to the 6th floor.")
    
    $ listened_dan_toph = True
    jump call_log_menu

label finish_call_logs:
    "You add both call logs to the journal."
...
label interrogate_dan:
    scene dan_room_interro with dissolve
...
label interrogate_toph:
    scene toph_interro with dissolve
...
label interrogate_austin:
    scene austin_interro with dissolve
...
label interrogate_chandler:
    scene chandler_interro with dissolve
...
label evaluate_final_accusation:
    ...
    scene police_station with dissolve
    show captain at right:
        zoom 0.7
    with moveinright
    pause 1.0

    # --- DYNAMIC ARREST DIALOGUES ---
    if all_suspects:
        mc "I'm arresting all of them. They're connected. The evidence doesn't point to one person — it points to all of them. It was never about one suspect. It was about all of them."
        mc "All suspects have been detained."
        mc "We know who held the knife. But he didn't get there alone. Someone led her there. Someone watched. Someone defiled her after."
        pc "They're all responsible."
        "The player leans forward."
        mc "One is a killer. One is a monster. One is a coward. And one lit the match."
        pc "So justice is served."
        "He pauses."
        mc "For Pat? Maybe. But cases like this? They don't really end. They just… wait."
        pc "For what?"
        "He doesn't answer. Just pushes the file aside."
        mc "For the next one."

    else:
        if end_toph_status == "Suspect":
            mc "I'm arresting Toph."
            mc "Toph had motive — the blackmail, the money, the drugs. But the knife was wiped clean. Almost like someone wanted us to find it there."
            pc "You think he was set up?"
            mc "I think Toph is guilty of a lot of things. But murder? The evidence is there… just not sure it tells the whole story."
            "Suspects have been detained. But something about this case still doesn't sit right with me."

        if end_chandler_status == "Suspect":
            mc "I'm arresting Chandler."
            mc "Chandler had rage. That much is clear. But Pat called him for help. She trusted him."
            pc "And he used that trust."
            mc "The wounds match someone who knew her. Someone angry. Someone personal."
            pc "That's Chandler."
            mc "Yes. But I get the feeling he wasn't alone in that room."
            "Suspects have been detained. But something about this case still doesn't sit right with me."

        if end_austin_status == "Suspect":
            mc "I'm arresting Austin."
            mc "Austin wasn't just watching. The lacerations on her chest — jagged, post-mortem. That wasn't the killer's work."
            pc "That was him."
            mc "Austin is a stalker. He's been watching her for weeks. And when he saw what happened… he wanted a piece of it."
            pc "So he's guilty."
            mc "Of desecration? Yes. But the killing blow? That came from someone else."
            "Suspects have been detained. But something about this case still doesn't sit right with me."

        if end_dan_status == "Suspect":
            mc "I'm arresting Dan."
            mc "Dan is a drug dealer. He led Pat to that room because he was mad — he wanted his money."
            pc "Toph owed him."
            mc "Toph owed him. And Pat was caught in the middle. Dan didn't plan a murder. He just wanted what he was owed."
            pc "That doesn't excuse him."
            mc "No. But he didn't kill her. He just opened the door."
            pc "And let someone else in."
            mc "And let someone else in."
            "Suspects have been detained. But something about this case still doesn't sit right with me."

        # Fail-safe just in case the player arrested literally nobody
        if not all_suspects and end_toph_status != "Suspect" and end_chandler_status != "Suspect" and end_austin_status != "Suspect" and end_dan_status != "Suspect":
            mc "I... I don't have enough to make an arrest yet."
